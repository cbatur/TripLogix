                        ...
                        VStack(spacing: 10) {
                            ...
                            HStack(spacing: 10) {
                                ForEach(place.result.photos.enumerated().filter { $0.offset % 2 == 0 }.map { $0.element }, id: \.photoReference) { photo in
                                    AsyncImage(
                                        url: URL(string: "https://maps.googleapis.com/maps/api/place/photo?maxwidth=400&photoreference=\(photo.photoReference)&key=AIzaSyAEuyVavDmSkQ7Tpc2M2Q-D2YUct4ewe7M")
                                    ) { phase in
                                        switch phase {
                                        case .empty:
                                            ProgressView()
